...
\textit{Lässt sich die 3D-Rekonstruktion linearer Infrastruktur durch eine vorgelagerte 2D-Segmentierung und objektspezifische 3DGS-Trainingsfunktionen so optimieren und georeferenzieren, dass strenge ingenieurstechnische Toleranzen ($\pm$ 10\,cm) eingehalten werden?}
...
\begin{figure}[h!]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}[node distance=1.3cm, auto,
            ...
            \node[box] (video) {Drohnen-Video (RAW)\\+ GNSS Messung};
            ...
            \node[box, below=1cm of colmap] (sts) {\textbf{Segment-then-Splat}\\Object-specific 3DGS};

            \node[box, below left=1cm and 0.2cm of sts] (sugar) {\textbf{SuGaR}\\Regularisierung + Meshing};
            \node[box, below right=1cm and 0.2cm of colmap] (cc) {\textbf{CloudCompare}\\Point Picking (Host)};

            \node[box, below=3cm of sts] (dgtal) {\textbf{DGtal} Centerline\\+ \textbf{GDAL} Georef. $\rightarrow$ UTM};
            ...
            \node[draw=black!50, fill=white, rectangle, rounded corners, anchor=north, inner sep=0.2cm, yshift=-0.7cm, font=\footnotesize] at (eval.south) {
                \begin{tabular}{ll}
                    ...
                    \textcolor{blue!80}{\rule{0.4cm}{0.25cm}}   & A: SAM 3 (CUDA 12.8) \\
            ...
            \draw[arrow] (pathA) |- (colmap);
            \draw[arrow] (pathB) |- (colmap);
...
\begin{itemize}[noitemsep]
    ...
    \item Evaluierung von SAM 3: Hauptweg (Video Predictor) vs. Fallback (SAHI).
    \item Integration von Segment-then-Splat und Nachweis des objektspezifischen Trainings.
\end{itemize}

\noindent\textbf{Bachelorarbeit (BA) -- Fokus Skalierung \& Metrik (10 Wochen):}
\begin{itemize}[noitemsep]
    \item \textbf{Feldarbeit:} Drohnenflug + parallele GNSS-Referenzmessung (GCPs \& Kabel-Scheitelachse).
    \item Georeferenzierung via Point Picking in CloudCompare.
    \item Centerline-Extraktion via DGtal auf dem Kabel-Sub-Mesh.
    \item \textbf{Evaluation:} RMSE und Hausdorff-Distanz der 3DGS-Centerline vs. GNSS-Referenz (B-Spline), Nachweis der $\pm$ 10\,cm Toleranz.
