...
\textbf{Praxis-Anforderung (TenneT):}\\
Gem\"a\ss{} eines Praxisbeispiels des \"Ubertragungsnetzbetreibers TenneT wird eine Genauigkeit von $\pm 10$\,cm von der Kabelachse in Lage und H\"ohe gefordert, sowie $5$\,cm f\"ur Kontrollpunkte. Diese Werte dienen als Benchmark.

\textbf{Zentrale Forschungsfrage (BA):}\\
\textit{L\"asst sich aus einem 3DGS-Modell \"uber SuGaR-Mesh-Extraktion und Centerline-Algorithmen eine geometrisch verwertbare 3D-Achslinie f\"ur r\"ohrenf\"ormige Infrastruktur ableiten, die den Praxisanforderungen entspricht, und wie genau ist diese im Vergleich zur Real-Time Kinematic Global Navigation Satellite System (RTK-GNSS)-Referenz?}
...
F\"ur die Isolation der Kabel-Gaussians werden \textbf{drei Methoden} systematisch verglichen. F\"ur einen ersten Prototypen wird zun\"achst nur die Farbfilterung verwendet, da diese am einfachsten umzusetzen ist. Bei Erfolg wird die Segmentierungsqualit\"at schrittweise gesteigert. Dies ergibt eine zus\"atzliche, methodisch wertvolle Forschungsfrage.
...
\end{lstlisting}
\begin{itemize}[noitemsep]
    \item[$\square$] Alternativer Scope: Automatische Georeferenzierung \"uber eindeutig GNSS-eingemessene Passpunkte.
...
\subsection{Finale Einrichtung und Kompilierung (Skyfall-GS)}
Nach der grundlegenden Installation von Python und PyTorch m\"ussen die spezifischen 3DGS-CUDA-Kernels (Rasterizer) kompiliert und das Projekt gestartet werden.

\textbf{1. Systemumgebung (Compiler) pr\"ufen:}\\
Nach der Installation von \textit{Visual Studio Build Tools 2022} (C++ Desktop) und dem \textit{CUDA Toolkit 11.8} muss das System neu gestartet werden. Anschlie\ss end in der Anaconda Prompt pr\"ufen:
\begin{lstlisting}
nvcc --version   # Muss CUDA 11.8 anzeigen
cl               # Muss den MSVC C++ Compiler erkennen
\end{lstlisting}

\textbf{2. Submodules (CUDA Kernels) kompilieren:}\\
Der \texttt{submodules}-Ordner im Skyfall-GS Repository muss initialisiert und installiert werden. Dieser Schritt nutzt \texttt{cl.exe} und \texttt{nvcc.exe}, um die spezifischen Grafik-Befehle f\"ur die RTX-GPU ma\ss geschneidert zu bauen.
\begin{lstlisting}
conda activate skyfall-gs
cd C:\Users\k66700\Skyfall-GS
git submodule update --init --recursive
pip install ./submodules/diff-gaussian-rasterization
pip install ./submodules/simple-knn
\end{lstlisting}

\textbf{3. Datensatz laden und Training starten:}\\
Das Test-Dataset wird z.\,B. mittels des installierten \texttt{huggingface\_hub}-Pakets geladen (gem\"a\ss{} Skyfall-GS README). F\"ur eigene Bilder muss zun\"achst das COLMAP-Skript (z.\,B. \texttt{convert.py}) ausgef\"uhrt werden. Danach startet das Training:
\begin{lstlisting}
python train.py -s ./data/test_szene
\end{lstlisting}
...
Die Kamera wird am Campus auf ca. \textbf{2\,m H\"ohe} gef\"uhrt; bei einer realen Drohnenh\"ohe von 10\,m ergibt sich ein \textbf{Skalierungsfaktor von 5$\times$}. 
Idealerweise soll auf einer echten TenneT-Baustelle gemessen werden (vorbehaltlich interner Kl\"arung). Da durch eine Skalierung bei gr\"o\ss erer Entfernung Kameraparameter abweichen und Pixel eher verschwimmen, sind echte Daten der skalierten Alternative auf dem Campus deutlich vorzuziehen. F\"ur den simulierten Fall erzeugen die Testobjekt-Durchmesser bei 2\,m die gleiche Pixelabdeckung (GSD) wie reale Kabel bei 10\,m Flugh\"ohe:
...
% ============================================================
\section{13. Technische Erkenntnisse der Prototyp-Implementierung (Stand Juni 2026)}

W\"ahrend der praktischen Aufsetzung der Pipeline-Container und der ersten Testl\"aufe wurden entscheidende technische Erkenntnisse gewonnen, die als wertvolle Arbeitsgrundlage f\"ur die Bachelorarbeit dienen:

\subsection{A. Container-Kompatibilit\"at und CUDA-Gegen\"uberstellung}
\begin{itemize}
    \item \textbf{Problem:} Die Integration von SAM 3.1, COLMAP, STS-Training (mit custom CUDA autograd-backprop) und SuGaR in eine einzige Docker-Umgebung scheitert an inkompatiblen Abh\"angigkeiten.
    \item \textbf{Erkenntnis:} Eine Service-Orientierung ist unverzichtbar.
    \begin{enumerate}
        \item \texttt{sam3-preprocess} l\"auft optimal mit modernem \textbf{PyTorch (CUDA 12.6)} und Python 3.12, um die C++ optimierten \texttt{sam3\_cu} Extensions fehlerfrei zu binden.
        \item \texttt{sts-training} ben\"otigt \textbf{CUDA 12.1} (PyTorch 2.3.1), um die CUDA-Submodule \texttt{depth-diff-gaussian-rasterization} und \texttt{simple-knn} ohne Build-Isolation (\texttt{--no-build-isolation}) stabil kompilieren zu k\"onnen.
    \end{enumerate}
\end{itemize}

\subsection{B. Behebung des Nerfstudio-Abh\"angigkeitskonflikts}
\begin{itemize}
    \item \textbf{Problem:} Die Installation von \textit{nerfstudio} mit dem Parameter \texttt{--no-deps} (um Versionskonflikte bei tiefen CUDA-Ressourcen zu umgehen) f\"uhrt bei der Ausführung des COLMAP-Datenimports (\texttt{object\_specific\_initialization.py}) zu wiederholten \texttt{ModuleNotFoundError} (z.\,B. \textit{appdirs}, \textit{rich}, \textit{tyro}, \textit{matplotlib}, \textit{scikit-image}, \textit{rawpy}, \textit{viser}).
    \item \textbf{Erkenntnis:} Diese Kernbibliotheken m\"ussen explizit vor \textit{nerfstudio} in Container C installiert werden. Das Dockerfile wurde robust angepasst.
\end{itemize}

\subsection{C. Hierarchische Masken-Generierung (Vorder-/Hintergrund-Trennung)}
\begin{itemize}
    \item \textbf{Design:} Segment-then-Splat (STS) erfordert zur Abgrenzung des Objekts vom umgebenden Trassensand eine hierarchische Maskenstruktur, bestehend aus drei Masken je Bildframe:
    \begin{enumerate}
        \item \texttt{middle.png}: Die urspr\"ungliche flache Maske aus dem SAM-3.1-Videotracking.
        \item \texttt{small.png} (\textbf{Erosion}): Mittels eines $5\times5$ gro\ss{}en, rechteckigen Strukturelements (\texttt{cv2.MORPH\_RECT}) wird die Maske erodiert, um einen geometrisch absolut sicheren Kernbereich des Objekts zu markieren.
        \item \texttt{default.png} (\textbf{Dilation}): Die Maske wird um 1 Iteration dilatiert, um einen gro\ss{}z\"ugigen \"au\ss{}eren Schutzbereich einzuschlie\ss{}en, der Fehlsplits am Objektrand verhindert.
    \end{enumerate}
\end{itemize}

\subsection{D. Python-G\"ultigkeitsbereich (Scope-Shadowing)}
\begin{itemize}
    \item \textbf{Problem:} Bei der nachtr\"aglichen Implementierung der morphologischen Transformationen kam es innerhalb des Vorverarbeitungsskripts \texttt{extract\_masks\_notebook_flow.py} zu einem \texttt{UnboundLocalError}.
    \item \textbf{Erkenntnis:} Python stuft einen Bezeichner als lokal ein, wenn er an beliebiger Stelle in einer Funktion gebunden wird (z.\,B. lokaler Import \texttt{import cv2} am Ende der Funktion). Wenn dieser Name weiter oben gelesen wird (z.\,B. \texttt{cv2.imread} bei der Frame-Validierung am Anfang), st\"urzt die Ausf\"uhrung ab. Alle Imports m\"ussen zwingend auf globaler Ebene (Modul-Kopf) deklariert werden.
\end{itemize}

\subsection{E. One-Click Bypass (Runtime-Optimierung)}
\begin{itemize}
    \item \textbf{L\"osung:} Da das SAM-Videosegmentieren und die COLMAP-SfM-Bildeinmessung bei 4K-Drohnenmaterial extrem rechenintensiv sind, wurde ein Bypass-Skript (\texttt{run\_from\_sts.sh}) implementiert. Damit kann das 3D-Gaussian-Splatting-Training ab Schritt 3 unkompliziert mit bereits kalkulierten Zwischenergebnissen neu gestartet werden.
\end{itemize}
