...
\begin{enumerate}
    ...
    \item \textbf{SAM 3 Tracking-Stabilität (VRAM) und Modellkonsistenz:} \\
        ...
        \item \textcolor{blue}{\textbf{[AKTUELLER ERKENNTNISSTAND: SAM 3.1 Laufzeitvalidierung, Stand 06/2026]}} \\
            Der SAM-Container wurde auf einen kompatiblen SAM-3.1-Stand fixiert (Commit \texttt{9f22cb976fb6e38dad5bb34940fad852dd897d0e}); zusätzliche Laufzeitabhängigkeiten (u. a. \texttt{decord}, \texttt{nvidia-pytriton}) wurden erfolgreich integriert. Funktional konnte der Notebook-Flow (\texttt{start\_session} $\rightarrow$ \texttt{add\_prompt} $\rightarrow$ \texttt{propagate\_in\_video} $\rightarrow$ \texttt{close\_session}) reproduzierbar ausgeführt werden. \\
            Gleichzeitig zeigte die aktuelle Voll-Propagation (ohne Chunking) weiterhin ein OOM-Risiko auf 16-GB-GPUs; nach OOM trat teils ein instabiler CUDA-Allocator-Zustand auf, weshalb Prompt-Versuche in demselben Lauf abgebrochen werden mussten. Ein expliziter Threshold-Test mit \texttt{output\_prob\_thresh = 0.35} sowie Prompt-Fallbacks (Textvarianten und Point-Fallback) lieferte im aktuellen Datensatz noch keine nichtleeren Masken (\texttt{nonempty = 0}). \\
            Daraus folgt methodisch: Der nächste Arbeitsschritt ist eine zweistufige Initialisierung nach dem Muster ``Detect first frame $\rightarrow$ Track initialized object'' (angelehnt an den ComfyUI-Workflow), ergänzt um strengere Session-Isolation pro Versuch, um OOM-Folgezustände sicher zu vermeiden.
