...
\begin{itemize}
    ...
    \item \textbf{Zwischenschritt 3.5 (Kabel-Isolierung \& Qualitätsfilterung):} Ein dediziertes Python-Skript \texttt{filter\_cable\_pc.py} filtert die dichte STS 3D-Punktwolke direkt vor der Übergabe an das Meshing-Tool. Es extrahiert gezielt die Kabel-Vordergrund-Splats auf Basis der gelernten Objekt-IDs (Standardwert auf mittlerer Ebene \texttt{obj\_id\_m == 1}) und wendet zusätzlich hochentwickelte Opazitäts- und Farbschwellenwert-Filter an, womit transparente Floater sowie schwarze Splat-Gase („Rauch-Artefakte“) mathematisch bereinigt werden, um SuGaR-Artefakte drastisch zu reduzieren.
