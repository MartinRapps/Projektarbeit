...
*   **Automatisierte Parameter:** Das FFMPEG-Pre-Processing erzwingt im Autopilot ein komprimiertes Arbeitsvideo im optimalen Kompromiss von $1920 \times 1080$ Pixeln bei $10$\,FPS, einem Constant Rate Factor (CRF) von $23$ und dem $x264$-Preset \texttt{medium}. Das STS-Training wird auf standardmäßig $7000$ Gesamt-Optimierungsiterationen (bzw. $5000$ Iterationen in Stage 2) mit klassischer Speicherallokation festgelegt.
...
\begin{itemize}
    ...
    \item \textbf{Zwischenschritt 3.5 (Kabel-Isolierung \& Qualitätsfilterung):} Das Skript \texttt{filter\_cable\_pc.py} erzeugt zusätzlich zur unveränderten Full-Scene-Punktwolke eine gefilterte Objektwolke \texttt{point\_cloud\_cable.ply}. Es selektiert die konfigurierte Ziel-Objekt-ID (im aktuellen Versuchsdatensatz \texttt{obj\_id\_m == 0}) und kann Opazitäts- sowie Farbschwellen anwenden. Die getrennte Ablage verhindert, dass ein gefilterter Objekt-Checkpoint versehentlich als unmaskierter Full-Scene-SuGaR-Baseline dient.
...
\begin{enumerate}[noitemsep]
    ...
    \item \textbf{Automatisierte OCR-Erfassung der Transformationsmatrix (Tesseract):} Da das fehlerfreie, manuelle Abtippen der 16 hochpräzisen Gleitkommazahlen der 4x4-Transformationsmatrix aus der CloudCompare-Konsole fehleranfällig und mühsam ist, wurde eine automatisierte optische Zeichenerkennung (OCR) integriert. In Container A (SAM 3 Preprocessing, Python 3.12) wurden die Systemabhängigkeit \texttt{tesseract-ocr} und die Python-Schnittstelle \texttt{pytesseract} nachgerüstet. Der Breakpoint des Master-Skripts fragt nun interaktiv: \textit{"Möchten Sie die Matrix per OCR einlesen?"}. Der Nutzer muss lediglich einen Screenshot des Matrixfeldes unter \texttt{data/01\_raw/matrix\_screenshot.png} ablegen. Das Skript \texttt{ocr\_matrix.py} bereinigt OCR-Ungenauigkeiten (wie z.\,B. das Verschlucken von Dezimalpunkten oder unvollständige Vorzeichen) vollautomatisch und speichert das Ergebnis formatiert als CSV-Matrix unter \texttt{data/04\_sfm/matrix.txt} ab. Erst nach erfolgreicher Sichtprüfung und Bestätigung des Nutzers fährt die Pipeline fort.
