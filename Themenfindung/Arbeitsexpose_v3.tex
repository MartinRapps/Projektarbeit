...
\begin{figure}[h!]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}[node distance=1.5cm, auto,
            ...
            \node[box, below=1.2cm of colmap] (sts) {\textbf{Segment-then-Splat (STS)}\\Object-specific 3DGS Training\\(Masken + SfM $\rightarrow$ Objekt-Gaussians)};

            \node[box, below left=1cm and 0.2cm of sts] (sugar) {\textbf{SuGaR}\\Geometrische Regularisierung\\+ Poisson Mesh-Extraktion};
            ...
            \node[box, below=3.2cm of sts] (dgtal) {\textbf{DGtal Centerline-Extraktion}\\(Kabel-Sub-Mesh $\rightarrow$ Lokale Kurve)};
            ...
            \node[draw=black!50, fill=white, rectangle, rounded corners, anchor=north, inner sep=0.3cm, yshift=-1cm] at (eval.south) {
                \begin{tabular}{ll}
                    ...
                    \textcolor{blue!80}{\rule{0.5cm}{0.3cm}}   & Container A (SAM 3, CUDA 12.8)     \\
...
\begin{itemize}
    \item \textbf{Container A (Vorverarbeitung):} Beinhaltet PyTorch 2.x und SAM 3. Benötigt das Nvidia Container Toolkit (CUDA 12.8).
...
Die Lösung ist eine konsequente Zweiteilung der 3D-Rekonstruktion: \textit{Segment-then-Splat} (Lu et al., NeurIPS 2025)\footnote{Y.~Lu et al., ``Segment then Splat: Unified 3D Open-Vocabulary Segmentation via Gaussian Splatting'', NeurIPS 2025. \url{https://github.com/luyr/Segment-then-Splat}} übernimmt als \textbf{Container C} das objektspezifische 3DGS-Training mit einem nativen Object-specific Loss ($\mathcal{L}_{obj}$), der die Ground-Truth-Bilder nicht manipuliert. STS teilt die initialen COLMAP-Gaussians \textit{vor} dem Training anhand der SAM 3 Masken in objektspezifische Sets auf und eliminiert damit sämtliche in den vorherigen Abschnitten beschriebenen Artefakte architektonisch. Allerdings basiert STS auf Vanilla 3DGS und liefert keine geometrische Regularisierung.

Daher wird der vortrainierte STS-Checkpoint anschließend an \textbf{SuGaR (Container D)} übergeben. SuGaR wurde exakt für diesen Workflow konzipiert: Es lädt einen bestehenden 3DGS-Checkpoint und wendet zusätzliche Regularisierungs-Losses (DN-Consistency, Normal Alignment) an, um die Gaussians flach an die reale Oberfläche zu zwingen. Abschließend extrahiert SuGaR via Poisson Surface Reconstruction ein hochqualitatives Mesh. Da jeder Gaussian durch STS bereits eine Object-ID trägt, kann das Kabel-Sub-Mesh nach dem Meshing anhand dieser IDs isoliert und an DGtal (Container E) zur Centerline-Extraktion übergeben werden.
