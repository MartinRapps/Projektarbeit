...
Der Leitungsbau (DC-Erdkabel, Überlandleitungen) durch Übertragungsnetzbetreiber (TSOs) erfordert eine exakte 3D-Dokumentation des Baufortschritts. In einem Praxisbeispiel fordert TenneT hierbei eine Genauigkeit von $\pm 10$\,cm von der Kabelachse in Lage und Höhe, sowie $5$\,cm für Kontrollpunkte. Klassische Photogrammetrie (Dense Multi-View Stereo, MVS) liefert hohe metrische Genauigkeit, hat jedoch Schwächen bei feinen Strukturen und erfordert lange Rechenzeiten. 3D Gaussian Splatting (3DGS) bietet fotorealistische Echtzeit-Renderings und bewahrt feine Details, ist aber für geodätische Zwecke an diesen Vorgaben kaum validiert.
...
Um Datenschutzhürden vorerst zu vermeiden, wird die Methodik zunächst auf einem \textbf{Campus-Testfeld} evaluiert. Gartenschläuche (Erdkabel) und Schnüre simulieren die Infrastruktur. Durch größere Entfernungen in der Realität ändern sich jedoch Kameraparameter und Pixel verschwimmen eher. Daher besteht der Wunsch, nach Klärung mit TenneT eigene Daten von einer echten Baustelle zu nutzen, was einer bloß skalierten Campus-Alternative deutlich vorzuziehen ist.
...
\begin{enumerate}[noitemsep]
    \item \textbf{Segmentierung} -- Für einen ersten Prototypen wird zunächst die einfachste Methode, die Farbfilterung, angewendet. Bei Erfolg wird die Segmentierungsqualität durch ML-Methoden (z.B. Gaussian Grouping) gesteigert.
...
Die Überführung vom lokalen 3DGS-Koordinatensystem in ein geodätisches Bezugssystem ist für den TSO-Anwendungsfall essenziell. Ein alternativer Scope fokussiert sich zudem auf die automatische Georeferenzierung über eindeutig GNSS-eingemessene Passpunkte. Generell sind zwei Wege denkbar:
