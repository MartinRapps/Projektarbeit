...
def frame_id_from_image_name(image_name: str) -> int | None:
...
def iter_mask_candidates(masks_dir: Path, image_name: str, level: str) -> Iterable[Path]:
    ...
    frame_id = frame_id_from_image_name(stem)

    if frame_id is not None:
        frame_name = f"frame_{frame_id:05d}"
        yield masks_dir / frame_name / f"{level}.png"
        yield masks_dir / "000" / f"{frame_id:05d}.png"
        if level == "default":
...
def load_mask(
    ...
    mask_path = next((path for path in iter_mask_candidates(masks_dir, camera.image_name, level) if path.is_file()), None)
